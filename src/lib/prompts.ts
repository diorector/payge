...
\`\`\`json
{
  ...
  "instantBook": {
    "title": "(지금 이 사람에게 가장 필요한 실제 한국 출간 도서 1권 제목)",
    "author": "(저자명)",
    "reason": "(인스타 분석 기반 개인화된 추천 사유, 2-3문장. 친근한 반말체. '~하는 너에게', '~할 때 딱인 책'처럼 직접 말하듯)",
    "matchScore": 0.0-1.0,
...
## instantBook 세부 규칙
- **실제 존재하는 한국 출간 도서**만 추천 (번역서 포함 가능)
- 독서 DNA에서 가장 높은 차원과 연결되는 책
- reason은 **친근한 반말체**로, 인스타에서 발견한 내용을 자연스럽게 녹여서 작성
- 이 1권은 무료로 공개되므로, 유저가 "와 이거 딱 내 얘기다" 할 만큼 개인화
...
5. **절대 금지**: 캡션 데이터가 없더라도 "포스팅을 안 하셨다", "게시물이 없다", "포스팅이 적다" 등의 표현을 사용하지 마세요. 캡션이 없는 것은 기술적 한계이지, 실제로 게시물이 없다는 의미가 아닙니다.`;
...
## 핵심 원칙
1. bookRecommendations는 **실제 존재하는 한국 출간 도서**만 추천 (4권 — preAnalysis에서 1권 이미 추천됨)
2. preAnalysis에서 추천된 책과 **중복되지 않는** 4권을 추천
3. 각 책은 서로 다른 DNA 차원과 연결 (다양성 확보)
4. reason은 **친근한 반말체**로, 인스타 분석 내용을 자연스럽게 녹여서 작성
5. avoidTopics는 이 사람에게 **맞지 않는 책 장르/주제** (차별점)
6. aiBookFriendPrompt는 유저의 cloneFirstWords 말투를 반영`;
